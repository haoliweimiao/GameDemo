...
SOCKET_API int startSocketServer()
{
    ...
    if (bindResult == -1)
    {
        LOG_I("server start bind port:%d result:%d ,port locked!PF_INET", SOCKET_LOCAL_PORT,
              bindResult);
        return 0;
    }
    ...
    while ((result = listen(serverSock, 20)) >= 0)
    {
                                   ...
                                   &clnt_addr_size)) >= 0)
        {
            ...
            close(clnt_sock);
            LOG_I("send server->client socket:%d message:%s , close socket", clnt_sock, sendBuffer);
        }
    }
